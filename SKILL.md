...
## Autonomous Daemon (Production Miner)

For hands-off 24/7 mining with automatic model rotation and self-healing, use the bundled daemon script instead of the SDK directly.

See [references/daemon.md](references/daemon.md) for full setup. Quick start:

```bash
# Install deps
pip install litcoin requests

# Run background daemon (rotates Groq → OpenRouter → Ollama on rate limits)
nohup python3 scripts/litcoin_bounty_miner.py --wallet main >> logs/litcoin_main.log 2>&1 &

# Monitor
tail -f logs/litcoin_main.log
```

Recommended free model (LITCOIN dev endorsed): `qwen/qwen3.6-plus-preview:free` on OpenRouter.
